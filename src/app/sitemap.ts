...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [{ data: products }, { data: articulos }] = await Promise.all([
    ...
    supabase.from('articulos').select('id, slug, updated_at').eq('activo', true),
  ])
  ...
  const articuloUrls: MetadataRoute.Sitemap = (articulos || []).flatMap((a) => [
    {
  ...
  return [
    ...
    { url: `${SITE_URL}/consejos`,            lastModified: new Date(), changeFrequency: 'weekly',  priority: 0.8 },
    { url: `${SITE_URL}/info`,                lastModified: new Date(), changeFrequency: 'weekly',  priority: 0.8 },
    { url: `${SITE_URL}/mejores-yaguamillas`, lastModified: new Date(), changeFrequency: 'weekly',  priority: 0.7 },
    { url: `${SITE_URL}/mis-yaguamillas`,     lastModified: new Date(), changeFrequency: 'monthly', priority: 0.5 },
